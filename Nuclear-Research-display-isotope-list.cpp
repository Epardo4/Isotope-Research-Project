...
string scientificNotation(double num){
	...
	//if the number is less than 0.001 make a string that displays it as scientific notation (e.g. 1.23e-10)
...
*/
Element makeElement(string name, int atomicNumber, string atomicSymbol, vector<Isotope> isotopes, vector<Isotope> sortedIsotopes){
	Element element;
	element.name = name;
	element.atomicNumber = atomicNumber;
	element.atomicSymbol = atomicSymbol;
	element.isotopes = isotopes;
	element.sortedIsotopes = sortedIsotopes;
	return element;
}
/*
...
/*Used for comparing two isotopes together by their meanLives*/
bool compare (Isotope i,Isotope j) { return (i.meanLife < j.meanLife); }
/*
	Sorts each element of v2 into v1
*/
vector<Isotope> sortVectors(vector<Isotope> v1, vector<Isotope> v2){
	int i = 0, j = 0;
	//using the compare method, this sorts each element in both vectors into one assuming both vectors are passed into the function already sorted
	...
	//if the vectors were not the same size (they likely were not), then put all the remaining elements at the end of the vector (These are all greater than every other element)
	if(i < v1.size()) return v1;
	else if(j < v2.size())
		v1.insert(v1.begin() + v1.size(), v2.begin() + j, v2.end());
	return v1;
}
/*
	Sorts the passed in vector using the built-in sort function and returns what was sorted
*/
vector<Isotope> sortIsotopes(vector<Isotope> toSort){
        vector<Isotope> toReturn = toSort;
        sort(toReturn.begin(), toReturn.end(), compare);
        return toReturn;
}
...
int main(){
	...
	elements[0] = makeElement("Hydrogen", 1, "H", hList, sortIsotopes(hList));
	//Helium
	...
	elements[1] = makeElement("Helium", 2, "He", heList, sortIsotopes(heList));
	//Lithium
	...
	elements[2] = makeElement("Lithium", 3, "Li", liList, sortIsotopes(liList));
	//Beryllium
	...
	elements[3] = makeElement("Beryllium", 4, "Be", beList, sortIsotopes(beList));
	//Boron
	...
	elements[4] = makeElement("Boron", 5, "B", bList, sortIsotopes(bList));
	//Carbon
	...
	elements[5] = makeElement("Carbon", 6, "C", cList, sortIsotopes(cList));
	//Nitrogen
	...
	elements[6] = makeElement("Nitrogen", 7, "N", nList, sortIsotopes(nList));
	//Oxygen
	...
	elements[7] = makeElement("Oxygen", 8, "O", oList, sortIsotopes(oList));
	//Flourine
	...
	elements[8] = makeElement("Flourine", 9, "F", fList, sortIsotopes(fList));
	//Neon
	...
	elements[9] = makeElement("Neon", 10, "Ne", neList, sortIsotopes(neList));
	//Sodium
	...
	elements[10] = makeElement("Sodium", 11, "Na", naList, sortIsotopes(naList));
	//Magnesium
	...
	elements[11] = makeElement("Magnesium", 12, "Mg",mgList, sortIsotopes(mgList));
	//Aluminum
	...
	elements[12] = makeElement("Aluminum", 13, "Al", alList, sortIsotopes(alList));
	//Silicon
	...
	elements[13] = makeElement("Silicon", 14, "Si",siList, sortIsotopes(siList));
	//Phosphorus
	...
	elements[14] = makeElement("Phosphorus", 15, "P",pList, sortIsotopes(pList));
	//Sulfur
	...
	elements[15] = makeElement("Sulfur", 16, "S",sList, sortIsotopes(sList));
	//Chlorine
	...
	elements[17 - 1] = makeElement("Chlorine", 17, "Cl", clList, sortIsotopes(clList));
	//Argon
	...
	elements[18 - 1] = makeElement("Argon", 18, "Ar", arList, sortIsotopes(arList));
	//Potassium
	...
	elements[19 - 1] = makeElement("Potassium", 19, "K", kList, sortIsotopes(kList));
	//Calcium
	...
	elements[20 - 1] = makeElement("Calcium", 20, "Ca", caList, sortIsotopes(caList));
	//Scandium
	...
	elements[21 - 1] = makeElement("Scandium", 21, "Sc", scList, sortIsotopes(scList));
	//Titanium
	...
	elements[22 - 1] = makeElement("Titanium", 22, "Ti", tiList, sortIsotopes(tiList));
	//Vanadium
	...
	elements[23 - 1] = makeElement("Vanadium", 23, "V", vList, sortIsotopes(vList));
	//Chromium
	...
	elements[24 - 1] = makeElement("Chromium", 24, "Cr", crList, sortIsotopes(crList));
	//Manganese
	...
	elements[25 - 1] = makeElement("Manganese", 25, "Mn", mnList, sortIsotopes(mnList));
	//Iron
	...
	elements[26 - 1] = makeElement("Iron", 26, "Fe", feList, sortIsotopes(feList));
	//Cobalt
	...
	elements[27 - 1] = makeElement("Cobalt", 27, "Co", coList, sortIsotopes(coList));
	//Nickel
	...
	elements[28 - 1] = makeElement("Nickel", 28, "Ni", niList, sortIsotopes(niList));
	//Copper
	...
	elements[29 - 1] = makeElement("Copper", 29, "Cu", cuList, sortIsotopes(cuList));
	//Zinc
	...
	elements[30 - 1] = makeElement("Zinc", 30, "Zn", znList, sortIsotopes(znList));
	//Gallium
	...
	elements[31 - 1] = makeElement("Gallium", 31, "Ga", gaList, sortIsotopes(gaList));
	//Germanium
	...
	elements[32 - 1] = makeElement("Germanium", 32, "Ge", geList, sortIsotopes(geList));
	//Arsenic
	...
	elements[33 - 1] = makeElement("Arsenic", 33, "As", asList, sortIsotopes(asList));
	//Selenium
	...
	elements[34 - 1] = makeElement("Selenium", 34, "Se", seList, sortIsotopes(seList));
	//Bromine
	...
	elements[35 - 1] = makeElement("Bromine", 35, "Br", brList, sortIsotopes(brList));
	//Krypton
	...
	elements[36 - 1] = makeElement("Krypton", 36, "Kr", krList, sortIsotopes(krList));
	//Rubidium
	...
	elements[37 - 1] = makeElement("Rubidium", 37, "Rb", rbList, sortIsotopes(rbList));
	//Strontium
	...
	elements[38 - 1] = makeElement("Strontium", 38, "Sr", srList, sortIsotopes(srList));
	//Yttrium
	...
	elements[39 - 1] = makeElement("Yttrium", 39, "Y", yList, sortIsotopes(yList));
	//Zirconium
	...
	elements[40 - 1] = makeElement("Zirconium", 40, "Zr", zrList, sortIsotopes(zrList));
	//Niobium
	...
	elements[41 - 1] = makeElement("Niobium", 41, "Nb", nbList, sortIsotopes(nbList));
	//Molybdenum
	...
	elements[42 - 1] = makeElement("Molybdenum", 42, "Mo", moList, sortIsotopes(moList));
	//Technetium
	...
	elements[44 - 1] = makeElement("Ruthenium", 44, "Ru", ruList, sortIsotopes(ruList));
	//Rhodium
	...
	elements[45 - 1] = makeElement("Rhodium", 45, "Rh", rhList, sortIsotopes(rhList));
	//Palladium
	...
	elements[46 - 1] = makeElement("Palladium", 46, "Pd", pdList, sortIsotopes(pdList));
	//Silver
	...
	elements[47 - 1] = makeElement("Silver", 47, "Ag", agList, sortIsotopes(agList));
	//Cadmium
	...
	elements[48 - 1] = makeElement("Cadmium", 48, "Cd", cdList, sortIsotopes(cdList));
	//Indium
	...
	elements[49 - 1] = makeElement("Indium", 49, "In", inList, sortIsotopes(inList));
	//Tin
	...
	elements[50 - 1] = makeElement("Tin", 50, "Sn", snList, sortIsotopes(snList));
	//Antimony
	...
	elements[51 - 1] = makeElement("Antimony", 51, "Sb", sbList, sortIsotopes(sbList));
	//Tellurium
	...
	elements[52 - 1] = makeElement("Tellurium", 52, "Te", teList, sortIsotopes(teList));
	//Iodine
	...
	elements[53 - 1] = makeElement("Iodine", 53, "I", iList, sortIsotopes(iList));
	//Xenon
	...
	elements[54 - 1] = makeElement("Xenon", 54, "Xe", xeList, sortIsotopes(xeList));
	//Cesium
	...
	elements[55 - 1] = makeElement("Cesium", 55, "Cs", csList, sortIsotopes(csList));
	//Barium
	...
	elements[56 - 1] = makeElement("Barium", 56, "Ba", baList, sortIsotopes(baList));
	//Lanthanum
	...
	elements[57 - 1] = makeElement("Lanthanum", 57, "La",laList, sortIsotopes(laList));
	//Cerium
    ...
    elements[58 - 1] = makeElement("Cerium", 58, "Ce", ceList, sortIsotopes(ceList));
	...
	elements[59 - 1] = makeElement("Praseodymium", 59, "Pr", prList, sortIsotopes(prList));
	//Neodymium
	...
	elements[60 - 1] = makeElement("Neodymium", 60, "Nd", ndList, sortIsotopes(ndList));
	//Promethium
	...
	elements[62 - 1] = makeElement("Samarium", 62, "Sm", smList, sortIsotopes(smList));
	//Europium
	...
	elements[63 - 1] = makeElement("Europium", 63, "Eu", euList, sortIsotopes(euList));
	//Gadolinium
	...
	elements[64 - 1] = makeElement("Gadolinium", 64, "Gd", gdList, sortIsotopes(gdList));
	//Terbium
	...
	elements[65 - 1] = makeElement("Terbium", 65, "Tb", tbList, sortIsotopes(tbList));
	//Dysprosium
	...
	elements[66 - 1] = makeElement("Dysprosium", 66, "Dy", dyList, sortIsotopes(dyList));
	//Holmium
	...
	elements[67 - 1] = makeElement("Holmium", 67, "Ho", hoList, sortIsotopes(hoList));
	//Erbium
	...
	elements[68 - 1] = makeElement("Erbium", 68, "Er", erList, sortIsotopes(erList));
	//Thulium
	...
	elements[69 - 1] = makeElement("Thulium", 69, "Tm", tmList, sortIsotopes(tmList));
	//Ytterbium
	...
	elements[70 - 1] = makeElement("Ytterbium", 70, "Yb", ybList, sortIsotopes(ybList));
	//Lutetium
	...
	elements[71 - 1] = makeElement("Lutetium", 71, "Lu", luList, sortIsotopes(luList));
	//Hafnium
	...
	elements[72 - 1] = makeElement("Hafnium", 72, "Hf", hfList, sortIsotopes(hfList));
	//Tantalum
	...
	elements[73 - 1] = makeElement("Tantalum", 73, "Ta", taList, sortIsotopes(taList));
	//Tungsten
	...
	elements[74 - 1] = makeElement("Tungsten", 74, "W", wList, sortIsotopes(wList));
	//Rhenium
	...
	elements[75 - 1] = makeElement("Rhenium", 75, "Re", reList, sortIsotopes(reList));
	//Osmium
	...
	elements[76 - 1] = makeElement("Osmium", 76, "Os", osList, sortIsotopes(osList));
	//Iridium
	...
	elements[77 - 1] = makeElement("Iridium", 77, "Ir", irList, sortIsotopes(irList));
	//Platinum
	...
	elements[78 - 1] = makeElement("Platinum", 78, "Pt", ptList, sortIsotopes(ptList));
	//Gold
	...
	elements[79 - 1] = makeElement("Gold", 79, "Au", auList, sortIsotopes(auList));
	//Mercury
	...
	elements[80 - 1] = makeElement("Mercury", 80, "Hg", hgList, sortIsotopes(hgList));
	//Thallium
	...
	elements[81 - 1] = makeElement("Thallium", 81, "Tl", tlList, sortIsotopes(tlList));
	//Lead
	...
	elements[82 - 1] = makeElement("Lead", 82, "Pb", pbList, sortIsotopes(pbList));
	//Bismuth
	...
	elements[83 - 1] = makeElement("Bismuth", 83, "Bi", biList, sortIsotopes(biList));
	// Polonium
	...
	elements[90 - 1] = makeElement("Thorium", 90, "Th", thList, sortIsotopes(thList));
	//Protactinium
	...
	elements[90] = makeElement("Protactinium", 91, "Pa", paList, sortIsotopes(paList));
	//Uranium
	...
	elements[92 - 1] = makeElement("Uranium", 92, "U", uList, sortIsotopes(uList));
	// Neptunium
	...
	//makes the vector containing all of the isotopes from every element in elements in ascending order by their mean-lives
	vector<Isotope> allSortedIsotopes = sortVectors(elements[0].sortedIsotopes, elements[1].sortedIsotopes);
	for(int i = 2; i < 119; i++){
		if(elements[i].sortedIsotopes.size() == 0) continue;
		else allSortedIsotopes = sortVectors(allSortedIsotopes, elements[i].sortedIsotopes);
	}
	//continues to ask for user input until 0 is entered at which point, the program returns 0 and exits
	string display = "Enter an atomic number or symbol or input 'q' to exit.\n(Enter \"Sorted-Isotopes\" to see a list of all the elements sorted by mean-life)\n";
	cout << display;
	string input;
	cin >> input;
	Element element;
	...
	while(input.compare("q") != 0){
		if(input.compare("Sorted-Isotopes") == 0){
			for(int i = 0; i < allSortedIsotopes.size(); i++)
				cout << toString(allSortedIsotopes[i]) << "\n";
			cout << display;
			cin >> input;
			continue;
		}
